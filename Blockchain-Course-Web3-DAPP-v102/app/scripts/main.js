...
function    doGetNodeStatus()  {
    ...
    web3.net.getListening(function(error, result){
        if(error) setData('get_peer_count',error,true);
            ...
            // Since connected lets get the syncing status
            web3.eth.getSyncing(  function(  error,  result ) {
                if(error){
                    setData('get_syncing_count',error,true);
                } else {
                    setData('get_syncing_count','Syncing status: '+result,(result == 0));
                }
            });
                // Since connected lets get the mining status
            web3.eth.getMining(  function(  error,  result ) {
                if(error){
                    setData('get_mining_count',error,true);
                } else {
                    setData('get_mining_count','Mining status: '+result,(result == 0));
                }
        });
        }
    });
